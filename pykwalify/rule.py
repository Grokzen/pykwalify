...
class Rule(object):
    ...
    def init(self, schema, path):
        ...
        if schema is not None:
            if "type" not in schema:
                ...
                if any([sa in schema for sa in sequence_aliases]):
                    ...
                    raise RuleError(u"key 'type' not found in schema rule : {}".format(path))
            else:
                if not isinstance(schema["type"], str):
                    raise RuleError(u"key 'type' in schema rule is not a string type : {}".format(path))
        ...
        for k, v in schema.items():
            if k in func_mapping:
                ...
                raise RuleError(u"Schema is only allowed on top level of schema file...")
            else:
                raise RuleError(u"Unknown key: {} found : {}".format(k, path))
    ...
    def init_func(self, v, rule, path):
        ...
        if not isinstance(v, str):
            raise RuleError(u"value for func keyword must be a string : {} : {}".format(v, path))
    ...
    def init_extensions(self, v, rule, path):
        ...
        if not isinstance(v, list):
            raise RuleError(u"Extension defenition should be a list : {} : {}".format(v, path))
    ...
    def init_matching_rule(self, v, rule, path):
        ...
        if v not in allowed:
            raise RuleError(u"Specefied rule in key : {} is not part of allowed rule set : {}".format(v, allowed))
        else:
    ...
    def init_type_value(self, v, rule, path):
        ...
        if not is_builtin_type(self._type):
            raise RuleError(u"type.unknown : {} : {}".format(self._type, path))
    ...
    def init_matching(self, v, rule, path):
        ...
        if str(v) not in valid_values:
            raise RuleError(u"matching value: {} is not one of {}".format(str(v), valid_values))
    ...
    def init_pattern_value(self, v, rule, path):
        ...
        if self._schema_str["type"] == "map":
            raise RuleError(u"map.pattern : pattern not allowed inside map : {} : {}".format(v, path))
        ...
        try:
            ...
            raise RuleError(u"pattern.syntaxerr : {} --> {} : {}".format(self._pattern_regexp, self._pattern_regexp, path))

    def init_enum_value(self, v, rule, path):
        log.debug(u"Init enum value : {}".format(path))

        if not isinstance(v, list):
            raise RuleError(u"enum.notseq")
        self._enum = v

        if is_collection_type(self._type):
            raise RuleError(u"enum.notscalar")

        lookup = set()
        for item in v:
            if not isinstance(item, self._type_class):
                raise RuleError(u"enum.type.unmatch : {} --> {} : {}".format(item, self._type_class, path))

            if item in lookup:
                raise RuleError(u"enum.duplicate : {} : {}".format(item, path))
    ...
    def init_range_value(self, v, rule, path):
        ...
        if not isinstance(v, dict):
            raise RuleError(u"range.notmap : {} : {}".format(v, path))
        if self._type not in ["str", "int", "map", "seq"]:
            raise RuleError(u"range.not-supported-type : {} : {}".format(self._type, path))
        ...
        for k, v in self._range.items():
            if k not in ["max", "min", "max-ex", "min-ex"]:
                raise RuleError(u"range.undefined key : {} : {}".format(k, path))

        if "max" in self._range and "max-ex" in self._range:
            raise RuleError(u"range.twomax : {}".format(path))
        if "min" in self._range and "min-ex" in self._range:
            raise RuleError(u"range.twomin : {}".format(path))
        ...
        if max is not None and not is_int(max) or is_bool(max):
            raise RuleError(u"range.max.notint : {} : {}".format(max, path))

        if min is not None and not is_int(min) or is_bool(min):
            raise RuleError(u"range.min.notint : {} : {}".format(min, path))

        if max_ex is not None and not is_int(max_ex) or is_bool(max_ex):
            raise RuleError(u"range.max_ex.notint : {} : {}".format(max_ex, path))

        if min_ex is not None and not is_int(min_ex) or is_bool(min_ex):
            raise RuleError(u"range.min_ex.notint : {} : {}".format(min_ex, path))

        if max is not None:
            if min is not None and max < min:
                raise RuleError(u"range.maxltmin : {} < {} : {}".format(max, min, path))
            elif min_ex is not None and max <= min_ex:
                raise RuleError(u"range.maxleminex : {} <= {} : {}".format(max, min_ex, path))
        elif max_ex is not None:
            if min is not None and max_ex < min:
                raise RuleError(u"range.maxexlemiin : {} < {} : {}".format(max_ex, min, path))
            elif min_ex is not None and max_ex <= min_ex:
                raise RuleError(u"range.maxexleminex : {} <= {} : {}".format(max_ex, min_ex, path))

    def init_ident_value(self, v, rule, path):
        log.debug(u"Init ident value : {}".format(path))

        if v is None or isinstance(v, bool):
            raise RuleError(u"ident.notbool : {} : {}".format(v, path))

        self._ident = bool(v)
        self._required = True

        if is_collection_type(self._type):
            raise RuleError(u"ident.notscalar : {} : {}".format(self._type, path))
        if path == "":
            raise RuleError(u"ident.onroot")
        if self._parent is None or not self._parent._type == "map":
            raise RuleError(u"ident.notmap : {}".format(path))

    def init_unique_value(self, v, rule, path):
        log.debug(u"Init unique value : {}".format(path))

        if not isinstance(v, bool):
            raise RuleError(u"unique.notbool : {} : {}".format(v, path))

        self._unique = v

        if is_collection_type(self._type):
            raise RuleError(u"unique.notscalar : {} : {}".format(self._type, path))
        if path == "":
            raise RuleError(u"unique.onroot")

    def init_sequence_value(self, v, rule, path):
        log.debug(u"Init sequence value : {}".format(path))

        if v is not None and not isinstance(v, list):
            raise RuleError(u"sequence.notseq : {} : {}".format(v, path))

        self._sequence = v

        if self._sequence is None or len(self._sequence) == 0:
            raise RuleError(u"sequence.noelem : {} : {}".format(self._sequence, path))
    ...
    def init_mapping_value(self, v, rule, path):
        ...
        if self._mapping:
            raise RuleError(u"mapping.multiple-use : {}".format(path))

        log.debug(u"Init mapping value : {}".format(path))

        if v is not None and not isinstance(v, dict):
            raise RuleError(u"mapping.notmap : {} : {}".format(v, path))

        if v is None or len(v) == 0:
            raise RuleError(u"mapping.noelem : {} : {}".format(v, path))
        ...
        for k, v in v.items():
            ...
            if k.startswith("regex;") or k.startswith("re;"):
                ...
                regex = k.split(";", 1)
                if len(regex) != 2:
                    raise RuleError(u"Malformed regex key : {}".format(k))
                else:
                    regex = regex[1]
                    try:
                        re.compile(regex)
                    except Exception as e:
                        raise RuleError(u"Unable to compile regex '{}' '{}'".format(regex, e))
    ...
    def init_default_value(self, v, rule, path):
        ...
        if is_collection_type(self._type):
            raise RuleError(u"default.notscalar : {} : {} : {}".format(rule, path, v))

        if self._type == "map" or self._type == "seq":
            raise RuleError(u"default.notscalar : {} : {} : {}".format(rule, os.path.dirname(path), v))

        if not isinstance(v, self._type_class):
            raise RuleError(u"default.type.unmatch : {} --> {} : {}".format(v, self._type_class, path))
